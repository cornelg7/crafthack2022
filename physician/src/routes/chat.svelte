<script lang='ts'>
  ...
  pageTitleStore.set(`${Math.random() > 0.5 ? '👩‍🔬' : '👨‍🔬'} Chat with a patient`);
  navBarBackArrowStore.set({isVisible: true, route: '/'});
  ...
  authStore.subscribe(async ({ isLoggedIn, user }) => {
    if (!isLoggedIn) {
      ...
      const patientDocChange = await getPatientDoc($currentPatientIdStore);
